...
while payload:
    ...
    for i in range(QUEUE_SIZE):
        try:
            ...
            # Adjust queue size (NOTE FOR TOMORROW: DO NOT INCREMENT STARTING FROM PACKET 1 IF TIME > 5)
            if QUEUE_MODE == 0 and PROCESSING < 5:
                queueCounter += 1
                QUEUE_SIZE += 1
            ...
            if int(snMsg) == 0:
                ...
                PAYLOAD_SIZE = int(length//(80/(PROCESSING-latency))) # PAYLOAD_SIZE = int(length//((95-PROCESSING)/(PROCESSING-latency)))
